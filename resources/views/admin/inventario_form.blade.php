...
<body class="text-gray-800 font-sans h-screen flex items-center justify-center">
    ...
    <div class="bg-white p-8 rounded-xl border border-gray-200 shadow-sm w-full max-w-lg">
        ...
        <form action="{{ isset($producto) ? url('/admin/inventario/actualizar/' . $producto->idProducto) : url('/admin/inventario/guardar') }}" method="POST">
            @csrf

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Nombre del Producto</label>
                <input type="text" name="nombre" value="{{ $producto->nombre ?? $producto->nombre ?? $producto->descripcion ?? '' }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Stock Disponible</label>
                <input type="number" name="stockDisponible" value="{{ $producto->stockDisponible ?? '' }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="concentracionCBD_THC" id="concentracionCBD_THC" value="{{ $producto->concentracionCBD_THC ?? '' }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
            </div>

            <div class="flex justify-end space-x-3">
                <a href="{{ url('/admin/inventario') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300 transition">Cancelar</a>
