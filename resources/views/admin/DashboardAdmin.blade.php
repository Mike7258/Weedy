...
<aside class="w-64 bg-white border-r border-gray-200 flex flex-col justify-between shrink-0">
    <nav class="p-4 space-y-1">
        ...
        <a href="{{ url('/admin') }}" class="flex items-center space-x-3 bg-red-100 text-red-800 px-4 py-2.5 rounded-lg text-sm font-semibold">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between">
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-2">{{ $totalProductos ?? 0 }}</p>
                </div>
                
                <!-- 2. Pacientes -->
                <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between">
                    <h3 class="text-gray-500 text-xs uppercase tracking-wider font-semibold">Pacientes Activos</h3>
                    <p class="text-2xl font-bold text-gray-900 mt-2">{{ $totalPacientes ?? 0 }}</p>
                </div>
                
                <!-- 3. Pedidos Activos -->
                <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between">
                    <h3 class="text-blue-500 text-xs uppercase tracking-wider font-semibold">Pedidos Activos</h3>
                    <p class="text-2xl font-bold text-blue-600 mt-2">{{ $pedidosActivos ?? 0 }}</p>
                </div>
                ...
                <div class="bg-white p-5 rounded-xl border border-red-200 shadow-sm flex flex-col justify-between">
                    ...
                    <p class="text-2xl font-bold text-red-600 mt-2">{{ count($productosBajoStock ?? []) }}</p>
                </div>
            </div>
                        ...
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($ordenesRecientes as $orden)
                                    <tr>
                                        <td class="py-3">{{ $orden->idPedido }}</td>
                                        <td class="py-3 font-semibold text-gray-900">{{ $orden->nombre_usuario ?? 'No registrado' }}</td>
                                        <td class="py-3">
                                            <span class="px-2 py-1 bg-gray-100 rounded-full text-xs font-semibold">
                                                {{ $orden->estadoEnvio ?? 'Pendiente' }}
                                            </span>
                                        </td>
                                        <td class="py-3 text-right">
                                            <!-- Botón de detalles -->
                                            <a href="{{ url('/admin/ordenes/detalles/' . $orden->idPedido) }}" 
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                    ...
                    @forelse($productosBajoStock as $prod)
                        <div class="flex justify-between items-center border-b py-2 text-sm">
                            ...
                            <span class="text-red-600 font-bold">{{ $prod->stockDisponible }}</span>
                        </div>
                    @empty
