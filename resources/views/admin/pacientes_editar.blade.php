    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            <div class="flex items-center space-x-4 mb-6">
                <a href="{{ url('/admin/pacientes') }}" class="text-gray-400 hover:text-weedyRed transition">
                    <i class="fa-solid fa-arrow-left text-xl"></i>
                </a>
                <h1 class="text-3xl font-bold text-gray-900">Editar Paciente: <span class="text-gray-500 font-medium">{{ $paciente->idUsuario }}</span></h1>
            </div>

            <!-- FORMULARIO DE EDICIÓN -->
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-8 max-w-4xl">
                <form action="{{ url('/admin/pacientes/actualizar/' . $paciente->idUsuario) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="nombreCompleto" value="{{ $paciente->nombreCompleto }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-weedyRed focus:border-weedyRed focus:outline-none transition">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Cédula de Identidad</label>
                                <input type="text" name="cedulaIdentidad" value="{{ $paciente->cedulaIdentidad }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-weedyRed focus:border-weedyRed focus:outline-none transition">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Correo Electrónico</label>
                                <input type="email" name="correoElectronico" value="{{ $paciente->correoElectronico }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-weedyRed focus:border-weedyRed focus:outline-none transition">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Teléfono</label>
                                <input type="text" name="telefono" value="{{ $paciente->telefono }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-weedyRed focus:border-weedyRed focus:outline-none transition">
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="documentosValidacion" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-weedyRed focus:border-weedyRed focus:outline-none transition">{{ $paciente->documentosValidacion }}</textarea>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Historial Clínico</label>
                                <textarea name="historialClinico" rows="4" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-weedyRed focus:border-weedyRed focus:outline-none transition">{{ $paciente->historialClinico }}</textarea>
                            </div>
                        </div>
                    </div>

                    <div class="flex justify-end space-x-3 pt-6 border-t border-gray-100">
                        <a href="{{ url('/admin/pacientes') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2.5 rounded-lg text-sm font-medium transition">
                            Cancelar
                        </a>
